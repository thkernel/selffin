...
class PrioritiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create priority" do
    assert_difference('Priority.count') do
      post priorities_url, params: { priority: { name: @priority.name, status: @priority.status, uid: @priority.uid, user_id: @priority.user_id } }
    end
  ...
  test "should update priority" do
    patch priority_url(@priority), params: { priority: { name: @priority.name, status: @priority.status, uid: @priority.uid, user_id: @priority.user_id } }
